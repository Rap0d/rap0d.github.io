...
## 관계대수 ($$\pi, \sigma, \bowtie, \div, \wedge, \cap, \cup, -$$)
 - Select  : 릴레이션에서 조건을 만족하는 튜플을 구하기 위한 연산
	* 표기형식 : <선택조건>(테이블 이름)
	* Ex :  점수 > 80 (학생) etc.
 - Project  : 릴레이션에서 속성의 값을 구하는 연산
	* 표기형식 : <추출 속성리스트>(테이블 이름)
	* Ex :  학번, 이름( 학년 = 1 (학생))
 - Join  : 두 테이블로부터 조건에 맞는 관련된 튜플들을 하나의 튜플로 결합하여 하나의 테이블로 만드는 연산
	* 표기형식 : 테이블1  <조인조건> 테이블2
	* Ex : 학생  학번=학번 성적
 - Division  : 'A DIVISION B'는 B 테이블의 조건을 만족하는 튜플들을 테이블 A에서 추출하는 연산
	* 표기형식 : 테이블1 (테이블1속성  테이블2속성) 테이블2
	* Ex : 학생(수강과목  과목명)과목
 - 일반집합연산자 : 합(), 교(), 차() 집합, 카디션 프로덕트()연산
...
관계 해석
 ...
 - 튜플 관계 해석과 도메인 관계 해석이 있음
	* 표기형식 : {결과값 | 조건}
	* Ex : {학점.이름 | 학점  학점.수강번호='B123'  학점.점수>80}